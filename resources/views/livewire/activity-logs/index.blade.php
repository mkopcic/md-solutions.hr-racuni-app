    ...
    <div class="lg:hidden space-y-4">
            ...
            <div class="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                <div class="mb-3 flex items-start justify-between">
                    <div class="flex-1">
                        ...
                        <p class="text-sm font-medium text-zinc-900 dark:text-white mb-1">{{ $activity->description }}</p>
                        <div class="flex items-center gap-4 text-xs text-zinc-500 dark:text-zinc-400">
                            <span><i class="fas fa-calendar"></i> {{ $activity->created_at->format('d.m.Y H:i:s') }}</span>
                        </div>
                    </div>
                </div>
                <div class="space-y-2 text-sm border-t border-zinc-200 dark:border-zinc-700 pt-3">
                    <div class="flex justify-between">
                        <span class="text-zinc-500 dark:text-zinc-400">Korisnik:</span>
                        @if($activity->causer)
                            <span class="font-medium text-zinc-900 dark:text-white">{{ $activity->causer->name }}</span>
                        @else
                            <span class="text-zinc-500 dark:text-zinc-400">System</span>
                        @endif
                    </div>
                ...
                @if($activity->properties->isNotEmpty() || $activity->subject || $activity->causer)
                    <div class="mt-3">
                        <button
                            wire:click="showDetails({{ $activity->id }})"
                            type="button"
                            class="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-3 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-colors"
                        >
                            <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        ...
        @empty
            <div class="rounded-lg border border-zinc-200 bg-white p-8 text-center dark:border-zinc-700 dark:bg-zinc-900">
                <i class="fas fa-history text-4xl text-zinc-300 dark:text-zinc-600 mb-3"></i>
                <p class="text-zinc-500 dark:text-zinc-400">Nema aktivnosti za prikaz</p>
            </div>
        @endforelse
    </div>
