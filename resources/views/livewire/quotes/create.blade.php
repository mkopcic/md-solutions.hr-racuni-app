<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Nova ponuda</h1>
            <p class="text-zinc-600 dark:text-zinc-400">Kreiranje nove ponude</p>
        </div>
    </div>
    ...
    <form class="space-y-6">
        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="grid gap-6 md:grid-cols-3">
                <div>
                    <label for="quote_type" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Vrsta ponude <span class="text-red-500">*</span></label>
                    <select wire:model.live="quote_type" id="quote_type" class="w-full rounded-lg border border-zinc-300 bg-white p-2.5 text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white dark:placeholder-zinc-400 dark:focus:border-blue-500 dark:focus:ring-blue-500">
                        <option value="R">Ponuda (R)</option>
                        <option value="RA">Ponuda avansna (RA)</option>
                        <option value="P">Predponuda (P)</option>
                    </select>
                    @error('quote_type') <span class="mt-1 text-sm text-red-600">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label for="quote_number" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Broj ponude <span class="text-red-500">*</span></label>
                    <input type="number" wire:model="quote_number" id="quote_number" readonly class="w-full rounded-lg border border-zinc-300 bg-zinc-100 p-2.5 text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-800 dark:text-white dark:placeholder-zinc-400 dark:focus:border-blue-500 dark:focus:ring-blue-500">
                    @error('quote_number') <span class="mt-1 text-sm text-red-600">{{ $message }}</span> @enderror
                    <p class="mt-1 text-xs text-zinc-500">Format: {{ $quote_number }}-1-1</p>
                </div>
            ...
            <div class="mt-6 grid gap-6 md:grid-cols-2">
                ...
                <div>
                    <label for="valid_until" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Vrijedi do <span class="text-red-500">*</span></label>
                    <input type="date" wire:model="valid_until" id="valid_until" class="w-full rounded-lg border border-zinc-300 bg-white p-2.5 text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white dark:placeholder-zinc-400 dark:focus:border-blue-500 dark:focus:ring-blue-500">
                    @error('valid_until') <span class="mt-1 text-sm text-red-600">{{ $message }}</span> @enderror
                </div>
            </div>
            ...
            <div class="mt-6 grid gap-6 md:grid-cols-2">
                <div>
                    ...
                    <textarea wire:model="note" id="note" rows="3" class="w-full rounded-lg border border-zinc-300 bg-white p-2.5 text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white dark:placeholder-zinc-400 dark:focus:border-blue-500 dark:focus:ring-blue-500" placeholder="Napomena koja će biti prikazana na ponudi"></textarea>
                </div>

                <div>
                    <label for="internal_notes" class="mb-2 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Interne napomene</label>
                    <textarea wire:model="internal_notes" id="internal_notes" rows="3" class="w-full rounded-lg border border-zinc-300 bg-white p-2.5 text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white dark:placeholder-zinc-400 dark:focus:border-blue-500 dark:focus:ring-blue-500" placeholder="Ako je ponuda plaćena avansom, dodajte bilješku ovdje"></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900 md:p-6">
            <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <h3 class="text-lg font-medium text-zinc-900 dark:text-white">Stavke ponude</h3>
                ...
                <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                    ...
                    <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-700 dark:bg-zinc-900">
                        @foreach ($items as $index => $item)
                            <tr wire:key="desktop-item-{{ $index }}">
                <td class="px-4 py-2">
                    <div class="space-y-2">
            ...
            <div class="space-y-4 md:hidden">
                @foreach ($items as $index => $item)
                    <div wire:key="mobile-item-{{ $index }}" class="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-800">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:justify-between">
            <a href="{{ route('quotes.index') }}" class="w-full rounded-lg border border-zinc-300 bg-white px-5 py-2.5 text-center text-sm font-medium text-zinc-700 hover:bg-zinc-100 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 sm:w-auto" wire:navigate>
                Odustani
            </a>

            <div class="flex flex-wrap gap-2">
                <button type="button" wire:click="saveAsDraft" class="w-full rounded-lg border border-zinc-300 bg-white px-5 py-2.5 text-center text-sm font-medium text-zinc-700 hover:bg-zinc-100 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700 sm:w-auto">
                    Spremi kao nacrt
                </button>

                <button type="button" wire:click="saveAndSend" class="w-full rounded-lg bg-blue-600 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 sm:w-auto">
                    Spremi i pošalji
                </button>
            </div>
        </div>
