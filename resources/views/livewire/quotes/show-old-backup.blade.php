<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <!-- e-Ponud Status Badge -->
            <div class="mt-2">
                @if ($quote->latestEracunLog)
                    @php
                        $eLog = $quote->latestEracunLog;
                        $eStatus = $eLog->status;
                    @endphp
        ...
        <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
            ...
            @php
                $totalPaid = $quote->paid_cash + $quote->paid_transfer;
                $isMathPaid = $totalPaid >= $quote->total_amount;
            @endphp

            @if ($isMathPaid && !$quote->isPaid())
                <button wire:click="syncStatus" wire:confirm="Želite li označiti ovaj ponud kao plaćen?"
    ...
    <div class="grid gap-6 md:grid-cols-3">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <p class="text-zinc-900 dark:text-white">
                                {{ \Carbon\Carbon::parse($quote->due_date)->format('d.m.Y') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-6">
            <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="mb-6">
                    <span
                        ...
                        @if ($quote->status === 'paid') bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                        @elseif($quote->status === 'partial')
                    ...
                    @if ($quote->status === 'paid' || $quote->status === 'partial')
                        <div class="mt-2">
                            <p class="text-sm text-zinc-600 dark:text-zinc-400">
                                Plaćeno:
                                {{ number_format($quote->paid_cash + $quote->paid_transfer, 2, ',', '.') }} € od
                            ...
                            <div class="mt-1 h-2 rounded-full bg-zinc-200 dark:bg-zinc-700">
                                <div class="h-2 rounded-full bg-green-500"
                                    style="width: {{ min(100, (($quote->paid_cash + $quote->paid_transfer) / $quote->total_amount) * 100) }}%">
                                </div>
                            </div>
                        </div>
                    @endif
                ...
                <div class="mt-4 divide-y divide-zinc-200 dark:divide-zinc-700">
                    <div class="flex justify-between py-2">
                        ...
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ number_format($quote->total_amount, 2, ',', '.') }}
                            €</span>
                    </div>
                    ...
                    <div class="flex justify-between py-2">
                        ...
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ number_format($quote->paid_cash, 2, ',', '.') }}
                            €</span>
                    </div>
                    ...
                    <div class="flex justify-between py-2">
                        ...
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ number_format($quote->paid_transfer, 2, ',', '.') }}
                            €</span>
                    </div>
                    ...
                    <div class="flex justify-between py-2">
                        ...
                        <span
                            class="font-medium text-zinc-900 dark:text-white">{{ number_format($quote->getRemainingAmount(), 2, ',', '.') }}
                            €</span>
                    </div>
                </div>
            ...
            <!-- e-Ponud Status -->
            <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
                <h2 class="mb-4 text-xl font-semibold text-zinc-900 dark:text-white">e-Ponud Status</h2>

                @if ($quote->latestEracunLog)
                    @php
                        $log = $quote->latestEracunLog;
                    @endphp
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                        ...
                        <div class="mt-4 flex flex-col gap-2">
                            ...
                            <a href="{{ route('eracun.outgoing.index') }}" class="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2 text-center text-sm font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700" wire:navigate>
                                <i class="fas fa-list"></i> Svi e-Ponude
                            </a>
                        </div>
                    </div>
                @else
                    <div class="text-center">
                        <p class="mb-4 text-sm text-zinc-600 dark:text-zinc-400">Ponud još nije poslan na FINA e-Ponud sustav</p>
                        <button wire:click="sendToEracun" wire:confirm="Želite li poslati ovaj ponud na FINA e-Ponud sustav?" class="w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
                            <i class="fas fa-paper-plane"></i> Pošalji e-Ponud
                        </button>
                        <a href="{{ route('eracun.outgoing.index') }}" class="mt-2 block text-sm text-blue-600 hover:text-blue-700" wire:navigate>
                            Pregled svih poslanih e-ponuda
                        </a>
                    </div>
                @endif
            </div>
            ...
            <form id="payment-form">
                ...
                <div class="mb-4">
                    ...
                    <small class="mt-1 text-xs text-zinc-500">Preostalo:
                        {{ number_format($quote->total_amount - ($quote->paid_cash + $quote->paid_transfer), 2, ',', '.') }}
                        €</small>
                </div>
